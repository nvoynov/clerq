...
describe CheckNodes do
  class SpecCheck < CheckNodes
    attr_writer :node
  ...
  describe '#nonuniq_ids' do
    it 'must return empty hash when not found any' do
      interactor.node = node
      interactor.nonuniq_ids.must_be_kind_of Hash
      interactor.nonuniq_ids.must_be_empty
    end

    it 'must retrun nonuniq ids' do
      node << Node.new(id: node.id, meta: {file_name: 'spec.1.md'})
      interactor.node = node
      interactor.nonuniq_ids.must_equal({'root' => ['spec.md', 'spec.1.md']})
    end
  end
  ...
  describe '#unknown_parents' do
    ...
    it 'must return unknown parents' do
      ...
      interactor.node = node
      interactor.unknown_parents.must_equal({'0' => ['10', '20']})
    end
  end
  ...
  describe '#unknown_references' do
    it 'must return empty hash when not found any' do
      interactor.node = node
      interactor.unknown_references.must_be_kind_of Hash
      interactor.unknown_references.must_be_empty
    end
    ...
    it 'must return unknown references' do
      ...
      interactor.node = node
      interactor.unknown_references.must_equal(
  ...
  describe '#unknown_order_index' do
    it 'must return empty hash when not found any' do
      interactor.node = node
      interactor.unknown_order_index.must_be_kind_of Hash
      interactor.unknown_order_index.must_be_empty
    end
    ...
    it 'must return unknown order index' do
      ...
      interactor.node = node
      interactor.unknown_order_index.must_equal(
        {'root' => ['adm', 'usr']})
    end
  end
